fileAttente <- function (lambda, mu, N, p1, p2, p3, t, demPlot){


	print('Vérification des paramêtres: .')
	stopifnot(p1+p2+p3==1)
	print('.')
	...
	nbOp <- 0
	#Somme des requêtes dans la file aprés chaque opération (pour la moyenne)
	somReq <- 0
	
	# initialisation du tps à 0
	tps <- 0
	
	# S'il y a eu une demande de plot, on prépare le plot
	if(demPlot) {
		# Axe du temps
		xT <- 0
	...
	while(tps<t){
		...
		if(demPlot) { # S'il y a eu une demande de plot
			xT = c(xT,tps)
			yR = c(yR,reqQ)
			yRP = c(yRP,reqPrio)
			yRN = c(yRN,reqNorm)
	...
	if(demPlot) { # Si on veut plotter
	  #Légende du graph
	  plot(xT, yR, xlab="time", ylab="Xt", type="s", main="Nombre de requetes dans le système") # On plote
